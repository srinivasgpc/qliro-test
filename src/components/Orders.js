...
import { Box, TableBody, TableContainer, TableFooter, Table, TableCell, TableRow, TableHead, TablePagination, Typography } from "@material-ui/core"
...
const Orders = ({ allOrders }) => {
  ...
  const OrderNumber = ({ type }) => {
    ...
    return <div className={"rotate_verticle " + color_type}>{type}</div>
  }
  ...
  const PaymentMethod = ({ payment_type, payment_method }) => {
    ...
    if (payment_type === "Paypal" || payment_type === "American Express" || payment_type === "Visa") {
      paymentType = payment_type === "Paypal" ? Paypal : payment_type === "Visa" ? Visa : Amex
    ...
    <Box className="orders_table">
      <Box display="flex" justifyContent="space-between" alignItems="center">
        ...
        <Box className="orders_table_help">
          <InfoOutlinedIcon className="orders_table_help_icon" /> <span className="orders_table_help_text"> Help</span>
        </Box>
      </Box>
      <TableContainer>
        <Table className={classes.table} aria-label="customized table">
          ...
          <TableBody>
            {(rowsPerPage > 0 ? tabledata.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage) : tabledata).map((row) => (
              <StyledTableRow key={row.id} className="orders_table_row">
                ...
                <StyledTableCell align="left">
                  <PaymentMethod payment_method={row.payment_method} payment_type={row.payment_type} />
                </StyledTableCell>
                <StyledTableCell align="left">
